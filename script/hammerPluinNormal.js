    ...
    var log = null;
    var el=document.querySelector("#tapView");//获取你想要放大的图片<img>标签id
   ...
   // mc.on("rotatestart rotatemove", onRotate);//旋转事件
    ...
    function onTap(ev) {
      simpleTap = true;
      if (firstTap == false || showTally == false) {
        return;
      }
      setTimeout(function () {
        if (simpleTap == true) {
    ...
    function analyzeTapData(ev) {
      setTapPoint(ev,function (hiddenLeft,hiddenTop,tapPointX,tapPointY) {
        ...
        var tapScrPoint_x = ev.center.x;
        var tapScrPoint_y = ev.center.y;
    ...
    function setTapPoint(ev,callback) {
      ...
      console.log('translate_x=='+transform.translate.x);
      console.log('translate_y=='+transform.translate.y);
